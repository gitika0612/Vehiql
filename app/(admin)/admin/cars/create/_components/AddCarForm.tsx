...
import { addCar, CarData, processCarImageWithAI } from "@/actions/cars";
...
export const AddCarForm = () => {
    ...
    useEffect(() => {
        if (processImageResult?.success && processImageResult?.data) {
            ...
            if (uploadedAiImage) {
                const reader = new FileReader();
                reader.onload = (e) => {
                    const target = e.target; // Assign e.target to a variable
                    if (target && target.result) {  // Check if target and result are not null
                        const result = target.result as string;
                        setUploadedImages((prev) => [...prev, result]);
                    } else {
                        console.warn('FileReader failed or result is null');
                    }
                };
                reader.readAsDataURL(uploadedAiImage);
            } else {
                // Handle the case where uploadedAiImage is null
                console.warn("Uploaded image is null");
            }
    ...
    const onSubmit = async (data: CarFormData) => {
        ...
        // Handle the case where seats might be undefined
        const seats = data.seats === undefined || data.seats === null ? '' : data.seats;  // Convert undefined or null to empty string
        ...
        const carData: CarData = {
            ...
            seats: seats === '' ? null : parseInt(seats),  // Convert empty string to null, else convert to number
            status: data.status,
        };
