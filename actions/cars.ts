...
export async function processCarImageWithAI(file: File): Promise<AIResponse> {
  try {
    ...
    try {
      const carDetails: CarData = JSON.parse(cleanedText);
...
export async function addCar({ carData, images }: AddCarParams): Promise<{ success: boolean }> {
  try {
...
export async function getCars(search: string = ""): Promise<{ success: boolean; data?: any; error?: string }> {
  try {
    let where = {};
    if (search) {
      where = {
    ...
    const cars = await db.car.findMany({ where, orderBy: { createdAt: "desc" } });

    const serializedCars = cars.map((car) => serializeCarData(car, false));
...
export async function updateCarStatus(
  ...
  try {
    ...
    // Fallback if error is not an Error
    return { success: false, error: "An unknown error occurred" };
  }
}
